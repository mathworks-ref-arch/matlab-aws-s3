function [altered, nameUsed] = createBucket(obj, createArg)
    ...
    elseif isa(createArg, 'aws.s3.model.CreateBucketRequest')
        altered = false;
        nameUsed = char(createArg.Handle.getBucketName());
        write(logObj,'verbose',['Creating bucket: ',nameUsed]);
        obj.Handle.createBucket(createArg.Handle);
    else
        write(logObj,'error','Expected argument of type character vector or aws.s3.model.CreateBucketRequest');
    end
end %function
